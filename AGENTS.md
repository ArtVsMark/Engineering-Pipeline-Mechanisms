...
| механизма, ОТМЕЧАЮЩЕГО пункт задачи после слияния | изменение называет закрытый пункт строкой в теле, и гейт это читает, но `- [x]` в задаче ставит человек: отметка не едет вместе с работой до конца | [#26](../../issues/26) |
...
| здоровья общей ветки | контур 3 договора не реализован: тревоги, перезапуска одной проверки, заморозки и разморозки нет — красноту видит человек | [#2](../../issues/2) |
