...
| **первого прогона публикации** | `badges.yml` написан, ветки `badges` ещё нет: факты и значок появятся после слияния в общую ветку, и до тех пор витрина без значка. Производное по-прежнему не коммитится туда, где живёт источник | [#2](../../issues/2) |
