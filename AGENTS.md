...
|---|---|---|
| сверки контекста с защитой ветки | `required-context` без токена владельца говорит «не выполнено»; расхождение имени с настройкой не ловится ничем другим | [#196](../../issues/196) |
| механизма у части правил, признанных действующими | они держатся практикой или построением: ответ есть, а гейта нет, и нарушение никто не поймает. Какие именно — печатает шаг `debt` | [#196](../../issues/196) |
| адресата у совещательного красного, кроме `review` | класс `advisory` уже объявляется данными, а механизм записи есть только у ревью: результат прочей совещательной проверки живёт в выводе шага и слияния не переживает | [#196](../../issues/196) |
| доказательства, что гейт краснел ДО починки | гейт требует от починки проверку, но видит только зелёный набор: краснел ли он на подделанном входе до правки, из дерева не прочесть — это держится порядком работы окна | [#196](../../issues/196) |
| адресата у **забытого** `hold` | метку читает очередь и снимает помеченное с неё, но разбудить по стоящему неделями `hold` некому: шаг печатает остановленное и на этом всё | [#196](../../issues/196) |
| **застрявших изменений** — шага `stuck-prs` | конвейер их не обходит: открытое изменение с красным или конфликтом видно только шагом `debt` — он печатает первые источники контура работы на каждом изменении. Обхода по сроку, «висит дольше названного», нет; тревога о застрявшем оставлена службе наблюдения решением [`004`](docs/decisions/004-schedules-stay-service-observes.md) | [#196](../../issues/196) |
| **автоперезапуска мигнувшей на ИЗМЕНЕНИИ** | видно его теперь: детектор пишет мигания изменений в реестр #99 рядом с миганиями общей ветки. Чего нет — самого перезапуска: разрешённый список пуст до первой записи детектора, и заполнять его догадкой хуже, чем не заполнять. Обязательные не перезапускаются никогда. Решение и условие пересмотра — [`014`](docs/decisions/014-a-flake-must-be-visible-before-it-is-rerun.md) | [#196](../../issues/196) |
...
Общий транспортный слой — предмет ОТЛОЖЕННОЙ передачи ([#196](../../issues/196)), а не
...
Канал предложений — `.rules/proposals.json` ([#196](../../issues/196)).
...
Осталось руками — и **только это**, до [#196](../../issues/196):
