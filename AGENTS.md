...
| **застрявших изменений** — шага `stuck-prs` | конвейер их не обходит: открытое изменение с красным или конфликтом видно только шагом `debt` — он печатает первые источники контура работы на каждом изменении. Обхода по сроку, «висит дольше названного», нет; тревога о застрявшем оставлена службе наблюдения решением [`004`](docs/decisions/004-schedules-stay-service-observes.md) | [#2](../../issues/2) |
| **разморозки** общей ветки и дежурства по срокам | контур 3 реализован наполовину: `main-red` считает красноту, перезапускает ровно один упавший джоб и записывает остаток в задачу, но выхода из заморозки — шага `thaw`, эскалации по счётчику попыток и приёмки починки строже обычной нет. Живым прогоном механизм ещё не подтверждён | [#2](../../issues/2) |
...
Осталось руками — и **только это**, до [#11](../../issues/11):
