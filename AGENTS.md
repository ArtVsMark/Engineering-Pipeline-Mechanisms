...
| **перезапуска мигнувшей на ИЗМЕНЕНИИ** | на общей ветке он есть (`main_red.py`, право `actions: write` у прогона). На изменении не строится намеренно: разрешённый список пуст по построению — обязательные считают дерево, и их красное дефект, а совещательные слияния не держат. Замер 12.09 по ста прогонам: перезапусков ноль, «зелёное со второго раза» два и оба — гонка сводного гейта, уже починенная. Решение и условие пересмотра — [`013`](docs/decisions/013-a-rerun-on-a-change-waits-for-a-measured-flake.md) | [#196](../../issues/196) |
