...
| секрета `CLAUDE_CODE_OAUTH_TOKEN` | ревью не состоится: оба его шага предупреждают и выходят, изменение смотрит человек | [#17](../../issues/17) |
| **прогона ревью на этом изменении** | `track_progress` сверяет файл прогона с версией общей ветки, а `review.yml` там ещё нет: ревью придёт на следующем изменении, после слияния | [#17](../../issues/17) |
...
| **прогона очереди на живом изменении** | очередь и слияние написаны, но механизм считается работающим по ПРОГОНУ, а не по коду: пока прогона нет, слияние делает окно руками | [#53](../../issues/53) |
...
**Прямое указание владельца идёт перед долгом по правилам, но НЕ перед
источниками 0–3.** У владельца есть предмет, которого механизм не знает; но
если общая ветка красна, изменение конфликтует или находка ждёт разбора —
сначала это.
