...
| **эскалации по счётчику попыток** и дежурства по срокам | выход из заморозки есть и идёт СОБЫТИЕМ: `ci` на общей ветке → `workflow_run` → `automerge` перечитывает здоровье базы. Чего нет — предела попыток починки и перехода к владельцу после него: «чиним, пока не позеленеет» без предела не сходится, если причина не в коде (109) | [#196](../../issues/196) |
