...
| ответа по части записей каталога | они несут `unreviewed`: это объявленная очередь, а не решение. Сколько именно — печатает шаг `debt` и `facts.json` в ветке `badges`; числа здесь нет намеренно (005) | [#9](../../issues/9) |
