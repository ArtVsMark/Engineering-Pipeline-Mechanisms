...
| адресата у **забытого** `hold` | шаг 11 (`stuck.yml`) остановленное меткой видит и называет причину в выводе захода — но вывод захода адресатом не является, и в реестр оно не попадает НАМЕРЕННО: отличить забытый стоп-кран от осознанного нечем, пока метка не называет, ЧЕГО она ждёт. Такой договорённости у нас нет, и шаг не стал выдумывать её за владельца (046) | [#243](../../issues/243) |
