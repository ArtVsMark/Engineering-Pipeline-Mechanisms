...
| чего нет | следствие для окна | задача |
|---|---|---|
| гейта перед пушем одной командой | проверять нечем, кроме как руками | [#11](../../issues/11) |
| прогонов и обязательного контекста | зелёного не бывает; отсутствие красного ничего не доказывает | [#11](../../issues/11) |
| открытия изменения от лица владельца | изменение, открытое из окна, унесёт авторство приложения | [#12](../../issues/12) |
| меток в дереве | метка, поставленная руками, механизмом не читается | [#10](../../issues/10) |
| журнала и версии | фрагмент журнала класть **уже** нужно, собирать пока некому | [#13](../../issues/13) |
| ответа каталогу (`.rules/bindings.json`) | проект правилам каталога ещё не отвечает | [#9](../../issues/9) |
| кода механизмов | этап 1 не закрыт: до решений [#3](../../issues/3) и [#4](../../issues/4) код не пишется | [#2](../../issues/2) |
