...
| **прогона у трети объявленных исходов** | исход `EXIT_*` — обещание поведения, и оно не проверено, пока по нему не прогнали. Датированный замер 13.09.2026: из 102 объявленных исходов 35 не прогонялись ни разу, и большинство из них — третий, «не отработал», снаружи неотличимый от «чисто» (045). Пробел назван поимённо в `.rules/outcomes.json` и держится реестром `tests/test_outcomes_run.py`: список только УБЫВАЕТ, покрытая строка краснеет наравне с непокрытым исходом, а новый механизм приезжает со своими прогонами, а не с дописью в долг | [#243](../../issues/243) |
