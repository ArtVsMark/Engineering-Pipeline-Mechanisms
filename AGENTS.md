...
| **порога покрытия** | число публикуется значком на каждом слиянии, и счётчик наконец видит подпроцессы: 77%, а не 66%. Порога нет НАМЕРЕННО — у нас одна точка замера и ноль данных о том, как число движется; назначенный по одной точке порог был бы четвёртым за смену решением на непроверенном замере (044). Появляется, когда накопится ряд, и берётся из него: «не ниже достигнутого», двигаясь только вверх (050). Класс, который покрытие реально ловило, закрыт гейтом `check_new_is_tested.py` | [#196](../../issues/196) |
| **версии контракта, которая рвётся с тегом** | `do_release` пишет `CONTRACT_VERSION` на КАЖДОМ выпуске, а `next_after` поднимает минор всегда — то есть версия контракта растёт и там, где поверхность не тронута, и каждый такой рост однажды потребует перечитать ответы, которых ничто не отменяло. Требование «выпуск, не тронувший контракт, не порождает ничего» при этом не выполняется. Развязать числа нельзя молча: `version.py` сверяет тег с `CONTRACT_VERSION` и берёт из него MAJOR.MINOR без тегов. Решение за владельцем | [#196](../../issues/196) |
