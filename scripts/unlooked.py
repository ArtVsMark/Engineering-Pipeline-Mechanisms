...
def is_verification(comment: dict[str, Any]) -> bool:
    ...
    ПРЕДЕЛ ТОТ ЖЕ, ЧТО У `is_run_answer`: ответ верификатора, перенесённый до
    #825, лежит под меткой позднего взгляда, и здесь он не узнаётся — замер
    цепочек по старому отрезку засчитает процитированную в нём находку.
    """
