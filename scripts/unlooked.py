...
#: не запускался»: запись есть, условие вычислено и оказалось ложным. У нас
#: таких условий два — изменение из форка (секрета там нет по построению) и
#: правка самого файла прогона. Прежде и это падало в «вердикта нет», хотя
#: текст реестра обещал форку другое состояние. Нашёл внешний взгляд на #127.
STATE_SKIPPED: Final = "шаг взгляда пропущен условием — форк или правка самого прогона"
STATES: Final = (
    STATE_NONE,
    STATE_CUT,
    STATE_LATE,
...
def why_quiet(runs: list[dict[str, Any]]) -> str:
    ...
    if any(str(run.get("conclusion") or "") == "success" for run in ours):
        return STATE_SILENT
    # ЗАПИСЬ ЕСТЬ, А ВЕРДИКТА НЕТ — ЭТО ТРИ РАЗНЫХ СОСТОЯНИЯ, А НЕ ОДНО.
    # Прежде все они назывались «вердикта нет» — тем же словом, каким
    # называется отсутствие записи ВООБЩЕ. А зовут они к разному: незавершённый
    # прогон — подождать, отменённый — посмотреть, кто гасит, пропущенный —
    # прочитать условие шага. Уровнять их значило бы стереть различие, которое
    # уже видно в данных (046).
    ...
    if any(str(run.get("status") or "") != "completed" for run in ours):
...
def render_body(entries: dict[int, Entry], watermark: int) -> str:
    ...
    lines = [
        ...
        "  нет ключа `CLAUDE_CODE_OAUTH_TOKEN`; ревьюер промолчал. Смотреть надо",
        "  лог прогона — но искать в нём уже есть что (154);",
        f"* «{STATE_SKIPPED}» — условие шага вычислено и оказалось",
        "  ложным. Условий два: изменение из форка (секрета там нет по",
        "  построению) и правка самого файла прогона, на которой действие",
        "  отказывается работать по расхождению с общей веткой (152). Это НЕ",
        "  «прогон не запускался»: запись есть, и она говорит «пропущено»;",
