...
#: Знак экранирования: следующий за ним знак берётся как есть. Внутри одинарных
#: кавычек он сам буквален — там оболочка не толкует ничего.
ESCAPE: Final = "\\"

#: Строка ANSI-C, `$'…'`: закрывается одинарной кавычкой, но `\` внутри неё
#: толкуется, и `\'` кавычку не закрывает — в отличие от простых одинарных.
ANSI_C: Final = "$'"

#: Номер процесса оболочки. Разбирается раньше `$'`: в `$$'…'` второй `$`
#: принадлежит паре, а кавычка открывает простые одинарные.
PID: Final = "$$"


def commands(run: str) -> list[str]:
    """Команды блока `run:` — так, как их увидит оболочка.

    ОДИН ПРОХОД ПО ВСЕМУ БЛОКУ, А НЕ ПО СТРОКАМ, и это главное. Каждая граница
    команды здесь родилась находкой, и каждая — о том, что разбор знал одну
    форму записи из нескольких
    ...
    * **комментарий командой не считается** — проект объясняет границы версий
      строчным комментарием внутри `run: |`, и такая строка без признака
      установки вышибала шаг из служебных (#635);
    * **хвостовой комментарий срезается** — `pytest  # не забыть pip install`
      читался установкой (#641). Решётка начинает комментарий только первым
      знаком слова: `pkg@git+URL#egg=x` — адрес, а не пояснение (#657);
    * **связки режут строку** — `pip install X && pytest` это две команды, и
      вторая делает работу (#635); **связка в кавычках не режет** — `echo "a |
      b"` одна команда (#657);
    * **экранированный знак — не граница**: `echo a\\|b` одна команда, а `\\"`
      внутри двойных кавычек строку не закрывает. Внутри одинарных `\\`
      буквален, и `'a\\'` закрыт второй кавычкой (#660);
    * **хвостовой `\\` продолжает команду** на следующей строке, и пару «`\\` +
      перевод строки» оболочка просто вынимает: `pip install \\` и `ruff` на
      новой строке — одна установка, а не установка и неизвестная команда
      (#660). В комментарии хвостовой `\\` ничего не продолжает: комментарий
      кончается переводом строки;
    * **`$'…'` — строка ANSI-C**: `\\'` в ней кавычку не закрывает, и
      `pip install $'a\\'b' && pytest` — две команды, а не одна установка
      (#671). Соседняя `$"…"` — те же двойные кавычки: `$` здесь обычный знак;
      а `$$'…'` — номер процесса и простые одинарные кавычки, не ANSI-C (#709).

    ГРАНИЦА: тело `<<EOF` разбор не узнаёт — его строки читаются командами. На
    дереве такого нет (замер 24.09.2026: `<<` в блоках `run:` стоит дважды, оба
    раза внутри кавычек `echo "text<<$eof"`), а ошибка в эту сторону
    безопасна: лишняя «команда» не ставит зависимостей и шаг из служебных не
    выводит.

    Прежде это были три разбора — деление блока по строкам, срез комментария
    и деление по связкам, — и о `\\` не знал ни один: блок резался по переводу
    строки раньше, чем кто-либо смотрел на знак перед ним. Нашёл внешний
    взгляд на #660.

    ПОЧЕМУ НЕ `shlex`. Он отдаёт ТОКЕНЫ, и собрать из них команду обратно нельзя
    — кавычки теряются, и `echo "a | b"` превращался в две команды; а его
    правило комментария шире оболочкиного (#657). Здесь знаки команды берутся
    как есть, вынимаются только комментарий и продолжение строки
    ([045](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/045-no-silent-fallback.md)).
    Незакрытая кавычка тянется до конца блока — одной командой, а не пустотой.
    """
    found: list[str] = []
    piece: list[str] = []
    quote = ""
    ...
    while place < len(run):
        sign = run[place]
        if sign == ESCAPE and quote != "'":
            if run[place + 1 : place + 2] != "\n":
                piece.append(run[place : place + 2])
            place += 2
            continue
        if quote:
            # Закрывает последний знак открывшего: у `$'` это одинарная кавычка.
            quote = "" if sign == quote[-1] else quote
            piece.append(sign)
            place += 1
            continue
        # `$$` — номер процесса, а не начало `$'`: пара берётся целиком, и
        # следующая за ней кавычка открывает простые одинарные (`a3b8d12`).
        if run.startswith(PID, place):
            piece.append(PID)
            place += len(PID)
            continue
        if run.startswith(ANSI_C, place):
            quote = ANSI_C
            piece.append(ANSI_C)
            place += len(ANSI_C)
            continue
        if sign in QUOTES:
            ...
            place += 1
            continue
        if sign == "#" and (not piece or piece[-1].isspace()):
            end = run.find("\n", place)
            place = len(run) if end < 0 else end
            continue
        # Перевод строки вне кавычек режет команду так же, как связка.
        hit = next((one for one in ("\n", *JOINERS) if run.startswith(one, place)), "")
        if hit:
...
def report(days: dict[str, Any], bounds: Bounds, today: str) -> str:
    ...
    lines += [
        ...
        " ведёт реестр миганий. Второй счёт того же разошёлся бы с первым молча (022).",
        "",
    ]
    return "\n".join(lines)
