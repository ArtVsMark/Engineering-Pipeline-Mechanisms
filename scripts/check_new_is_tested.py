...
def told_by_tests(name: str, module: str, tests: str) -> bool:
    ...
    # ([195](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/195-a-narrowed-predicate-names-its-neighbour.md)).
    # СРАВНЕНИЕ — НЕ ПЕРЕДАЧА, и это третья редакция признака. Знак `=` перед
    # именем бывает и хвостом сравнения: `x == имя`, `x != имя`, `x >= имя`.
    # Вторая редакция их засчитывала, то есть чинила `имя=` и заводила `==имя`.
    # Поэтому `=` принимается только одиночным: перед ним не должно стоять
    # `=`, `!`, `<` или `>`. Нашёл внешний взгляд на #249.
    handed = r"(?:(?<![=!<>])=|[,(\[])"
