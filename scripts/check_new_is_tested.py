...
def told_by_tests(name: str, module: str, tests: str) -> bool:
    ...
    #
    # ТОЧКА, А НЕ ЛЮБОЙ ПРЕФИКС. Первая редакция дописала `[\w.]*`, и он съедал
    # что угодно: `@мойпорядок` засчитывался прогоном имени `порядок`. Это
    # ложное срабатывание — вторая ошибка предиката, и она дороже пропуска:
    # гейт зеленеет там, где имя не прогонялось вовсе. Нашёл взгляд тем же
    # обходом (`8f63099`), на изменении, которое первую половину и чинило.
