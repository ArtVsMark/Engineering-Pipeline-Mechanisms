...
def roles_for(files: list[str], table: dict[str, Any]) -> tuple[list[str], dict[str, list[str]]]:
    ...
    по факту (взгляд на #785). Кроме путей `ignored.except`: они зовут роли по
    `by_path`, как любой другой путь. Что в исключениях и почему, говорит сама
    таблица (`why`) — здесь список не повторяется, иначе он отстаёт от неё
    (взгляды на #796 и #803, 071).
    """
...
def roles_section(base: str, repo: str, number: int) -> str:
    ...
    try:
        ...
        return (
            ...
            f"`{paths.ROLES}`, чей вопрос изменение задевает, и назови это первой строкой.\n"
        )
