...
def unrecorded(
    ...
    """Заходы после записанного; отметки нет или она не найдена — от последнего полного.
    ...
    ровно то, от чего `last_look` и заведён (замер на #333).

    ПОЧЕМУ ОТ ПОСЛЕДНЕГО ПОЛНОГО, А НЕ ОДИН ПОСЛЕДНИЙ (взгляд на #867). После
    #848 последним заходом бывает проверка починки: она новых записей не
    заводит. Пиши уборка только её — отметка встала бы, а находки полного
    захода не легли бы в реестр никогда, и `look_mode.settled` счёл бы полный
    заход записанным. Отметка обязана значить «полный заход записан».
    """
    seen = looks(comments)
    ids = [one for one, _ in seen]
    if since is None or since not in ids:
        fulls = [place for place, (_, look) in enumerate(seen) if not is_fix_check(look)]
        return seen[fulls[-1] :] if fulls else seen[-1:]
...
def catch_up(
    ...
    `findings-write` и сама уборка. Тогда пишется всё от последнего полного
    захода (`unrecorded`): отметка без записи потеряла бы его находки
    навсегда. Отметка потеряна, когда её заход удалён из ленты, — тогда
    уборка лишь переставляет её на последний:
