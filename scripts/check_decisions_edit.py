...
#: Цель ссылки markdown: `](каталог/документ#якорь)`. КАТАЛОГ — НЕ СОДЕРЖАНИЕ
#: РЕШЕНИЯ, А ДОКУМЕНТ — СОДЕРЖАНИЕ. Когда документ, на который решение
#: ссылается, переезжает (#840), ссылку в записи надо перевести, иначе она ведёт
#: в никуда (022). Поэтому сравнивается цель без каталога: переезд того же
#: документа проходит, а перенаправление на ДРУГОЙ документ — `042-….md` на
#: `043-….md` — переписывает решение и краснеет (взгляд на #875). Текст ссылки и
#: упоминания путей в прозе остаются содержанием целиком.
#: Каталог снимается ТОЛЬКО у документа — цели на `.md`. Адрес площадки вида
#: `../../issues/12` сравнивается целиком: без каталога `issues/12` и `pull/12`
#: слились бы в `12`, и перенаправление с задачи на изменение прошло бы
#: (взгляд на #880).
#: ПРЕДЕЛ НАЗВАН (195): два разных документа с одним именем в разных каталогах —
#: `README.md` — гейт не различит: перенаправление между ними пройдёт.
LINK_TARGET_RE: Final = re.compile(r"\]\((?:[^)\s]*/)?(?P<document>[^)/\s]+\.md(?:#[^)\s]*)?)\)")
