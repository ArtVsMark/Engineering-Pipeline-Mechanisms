...
#: Метка плана работ. Живёт здесь, рядом с метками прочих живых задач, а не в
#: `work_plan`: её узнаёт и счёт пунктов (`items_left`), а тот стоит ниже
#: `work_plan` по импорту (через `debt`). Узнающий берёт её импортом, а не
#: буквами — правило 209.
PLAN_MARKER: Final = "<!-- work-plan: не удаляйте, по этой строке план находится снова -->"


def is_plan(body: str) -> bool:
    """Тело ли это плана работ — один признак на всех читателей метки плана.

    Сборщик плана узнавал его вхождением метки, счёт пунктов — началом тела:
    два читателя одной метки судили по-разному (взгляд на #828).
    """
    return PLAN_MARKER in body
...
def roles(path: Path = paths.ROLES) -> frozenset[str]:
    ...
    СПИСОК НЕ ПИШЕТСЯ ВТОРОЙ РАЗ. Состав ролей живёт в `docs/agent/roles.md`, и
