...
Шаг зовётся и тогда (условие `execution_file != ''` или исход `failure`/`cancelled` —
снятый по тайм-ауту заход файла тоже не отдаёт, `3d8d82e`) с
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.source:
        print(
            ...
            "оно упало или было снято раньше, чем агент его записал"
        )
        return EXIT_OK
    try:
