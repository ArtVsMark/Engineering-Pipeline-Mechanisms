...
(тайм-аут задания, замена новым прогоном той же головы), и шаг называет
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.source and args.outcome == CANCELLED:
        ...
        # (`timeout-minutes` стоит на уровне задания во всех пяти прогонах) и
        # отмена группой `cancel-in-progress`, когда по той же голове пошёл
        # новый прогон: агент не отказывал, его заход сменил следующий. Слова
        # отказа здесь назвали бы отказ, которого не было (`6af0b2a`).
        print(
            f"::warning::{args.call}: заход снят — задание отменено (тайм-аут задания "
            "или замена новым прогоном той же головы); это не отказ агента"
        )
        return EXIT_OK
    if not args.source:
        print(
