...
#:
#: ОДИНОЧНАЯ ВСТАВКА НЕ ПЕРЕСЕКАЕТ СТРОКУ, и это не украшение образца. Шаг
#: открытия склеивает тела ВСЕХ коммитов ветки, и `[^`]*` при непарном
#: бэктике тянулся до следующего бэктика — через абзацы и через границу
#: коммита. Замер: забытая кавычка в одном коммите съедала `Refs #12` из
#: другого, а строка снятия находки пропадала целиком. Потеря молчаливая, и
#: цена ей — ветка с честно названной задачей, которая не открывается.
#:
#: Непарный бэктик теперь не вырезает ничего: закрывающего на той же строке
#: нет — совпадения нет — текст остаётся целым. Тройная вставка ищет свой
#: закрывающий забор и при его отсутствии тоже не срабатывает.
