...
class Resolution:
    ...
    def twin_of(self) -> dict[str, str]:
        ...
        идёт за ней до следующего «дубль», — новый список источников. Решает
        РАЗМЕР группы, а не запятая: разделители у разбора любые — запятая,
        точка с запятой, пробел (`MARK_RUN_RE`), и «A дубль B C дубль D» — тоже
        две пары. Цель одна в своей группе — она сама становится источником:
        так пишется цепочка. Все отпечатки до первого «дубль» — источники
        (взгляд на #876).
...
def resolutions_in_all(texts: Iterable[str]) -> list[str]:
    """Строки снятия со всех тел ветки: новый отпечаток — одна запись.

    Строка с «дубль» — исключение: она едет целиком и может повторить уже
    снятые отпечатки ради связи (ниже, взгляд на #814).
    ...
    for text in texts:
        for record in resolutions_parsed(text):
            ...
            # (взгляд на #814). Повтор снятого безвреден: реестр снимает запись
            # один раз, а архив оставляет снятие первым и лишь дописывает
            # связь, если её ещё не было (`findings_archive.add_change`).
