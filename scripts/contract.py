...
Проза при этом верна и нужна (`docs/dev/release.md`), но проверить по ней нечего
...
BREAKING_MARKS: Final = (
    ...
    # его слияние. Договор объявлял это несовместимым, а примет таких слов не
    # было — и переход проходил молча. Нашёл внешний взгляд на #276.
    "проверка стала обязательной",
)
...
def differences(before: dict[str, Any], after: dict[str, Any]) -> list[str]:
    ...
    for name in sorted(set(was_checks) | set(now_checks)):
        ...
        if name not in was_checks:
            ...
            continue
        if now_checks[name] == policy.REQUIRED and was_checks[name] != policy.REQUIRED:
            # ПОДНЯТИЕ ДО ОБЯЗАТЕЛЬНОЙ НАЗЫВАЕТСЯ СВОИМИ СЛОВАМИ, а не стрелкой
            # между двумя `bool`: договор объявляет этот переход несовместимым —
            # у потребителя меняется то, что держит его слияние, — а `breaking()`
            # узнаёт переходы по СЛОВАМ, и «False → True» мимо него проходило
            # молча. Договор и механизм говорили разное. Нашёл внешний взгляд
            # на #276.
            found.append(f"проверка стала обязательной: «{name}»")
            continue
