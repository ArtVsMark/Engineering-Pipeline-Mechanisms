...
def touched(base: str) -> list[list[str]]:
    ...
    Выборка `база..HEAD` — с пределом, названным у `bodies_of` (195).
...
def bodies_of(base: str) -> str:
    ...
    ПРЕДЕЛ НАЗВАН (195), и он общий с `touched`: тела и файлы берутся из
    `база..HEAD`. Ветка, срезанная не от свежей общей ветки, а поверх чужой,
    уже слитой уплотнением, несёт её коммиты: их части попадут в счёт, а их
    «Смешение:» заглушит предупреждение. Отличить чужой коммит от своего по
    одному дереву нечем; договор окна режет ветку от свежей общей ветки
    (CLAUDE.md, «Ветка в облачном окне»). Предел стоит здесь, а не у метки:
    искать его будут у выборки (взгляд на #878).
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # а не трассировка мимо объявленных (039, взгляд на #871). И без
        # `--warn` тоже: навык close-a-finding зовёт счёт без ключа, и после
        # коммита с меткой просьба назвать причину звучала бы и там (#878).
        reason = declared(bodies_of(args.base)) if len(found) > 1 else ""
