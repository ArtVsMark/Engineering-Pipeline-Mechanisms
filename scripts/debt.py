...
#: Текст СОБИРАЕТСЯ из порога, а не повторяет его прозой: «больше суток» стояло
#: рядом с порогом в 26 часов и расходилось с ним на два часа. Число, вписанное
#: рядом с числом, устаревает первым (005). Нашёл внешний взгляд на #126.
STALE_NOTE: Final = (
    f"числам больше {int(STALE_AFTER.total_seconds() // 3600)} ч — "
    "ночной заход каталога, похоже, пропущен"
)
