    ...
    **гибнет в ожидании** события — группа держит одного ожидающего, и замер
    ...
    for change in queue:
        ...
        if not repair and awaits_look(repo, change, owner_token):
            ...
            continue
        # ВЗГЛЯД, ПРОПУЩЕННЫЙ НА КРАСНОЙ ГОЛОВЕ, ГОЛОВА ЕЩЁ ДОЛЖНА (#762). Спрашивается
        # только у ЗЕЛЁНОЙ головы (`STATE_MERGEABLE`): голова с идущими проверками
        # тоже доходит сюда, но её вердикта ещё нет, и перезапуск раньше него
        # позвал бы ворота на красное снова (находка на #771). Взгляд
        ...
        # без взгляда значило бы вернуть щель, которую закрывал #654. Вердикт
        # головы ворота читают тем же разбором, что очередь (`ci_complete`, #775).
        owed = (
            owed_look(repo, change, owner_token) if not repair and state in STATE_MERGEABLE else ""
        )
        if owed:
            if change.armed:
                take_back(
