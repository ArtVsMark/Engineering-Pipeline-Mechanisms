...
def branch_health(repo: str, sha: str, owner_token: str) -> list[str]:
    ...
    # На общей ветке часть проверок изменения законно не идёт вовсе: они
    # объявлены на событие изменения. Поэтому отсутствие записи здесь не отказ —
    # в отличие от головы кандидата, где предмет обязан быть найден.
    problems, _ = ci_complete.verdict(runs, required, "", strict_missing=False)
    return problems
...
def head_verdict(repo: str, change: Change, owner_token: str) -> tuple[list[str], bool]:
    ...
    return ci_complete.verdict(runs, required, "", strict_missing=True)
