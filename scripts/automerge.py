...
def name_the_stuck_head(
    ...
    body = "\n".join(
        (
            ...
            "",
            f"Прогон взгляда [{run}](../actions/runs/{run}) на голове `{change.head[:12]}` "
            f"пропущен, пока голова была красной, и уже перезапускался ({attempt} попытки). "
            ...
            "",
            "**Что сделать:** перезапустить этот прогон взгляда руками, когда голова зелёная, "
            "или толкнуть новую голову — тогда взгляд позовёт сам толчок.",
        )
    )
    if dry_run:
