...
каталога даёт 21 холостой прогон против 12 полезных на шести изменениях.
...
def on_the_shared_branch(runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    Цена ошибки была ровно такой: очередь на первом же живом прогоне сообщила
    «общая ветка красна» по трём пропускам и не сдвинулась. Нашёл это прогон, а
    не набор — механизм подтверждается прогоном, а не чтением
...
def worst_per_name(runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    (толчок и навешенная метка), а не спор механизмов. Замер: на первом живом
    заходе очередь отвергла изменение с шестью зелёными именами, потому что
    каждое было представлено дважды.
