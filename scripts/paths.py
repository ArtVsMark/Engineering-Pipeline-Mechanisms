...
#: Формат номера в нём. Разбирают его гейт версии, сборщик журнала и выпуск, и
#: разбор один (214) — здесь, уровнем выше всех, а не у одного из соседей (090).
#: Группы — мажор, минор, патч: выпуску нужны разряды, остальным — совпадение.
...
#: `dev/` — тому, кто ведёт и выпускает проект, `use/` — подключающему.
#: МЕХАНИЗМ, КОТОРЫЙ ЧИТАЕТ ДОКУМЕНТ, БЕРЁТ ПУТЬ ОТСЮДА; остальные упоминания —
#: проза модулей, промпт взгляда в `review.yml`, `.rules/review-roles.json`,
#: литералы тестов — остаются буквами, и при переносе их правят тоже. Забытое
#: место находит не память, а гейт:
#: `tests/test_doc_links.py::test_paths_named_outside_documents_exist` — путь,
#: которого нет, краснеет (взгляд на #875). Указатель по читателю — `docs/README.md`.
