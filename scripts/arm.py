...
def subject(repo: str, number: int, token: str) -> dict[str, Any]:
    ...
    except ghrest.NotFound as exc:
        # «ИЗМЕНЕНИЯ НЕТ» ПРИХОДИТ 404, А НЕ ПУСТЫМ ТЕЛОМ, и разбирать надо то,
        # что приходит: пустой словарь здесь был ветвью, которой площадка не
        # порождает, — то есть непроверяемой. Нашёл внешний взгляд на #235.
        raise NotRun(
            f"#{number}: изменения у площадки нет — предмет замера не найден (075)"
        ) from exc
    if not full:
        # Пустое тело при коде 200 — форма ответа изменилась: не «нет
        # изменения», а «читать нечего», и это другой разговор (045).
        raise NotRun(f"#{number}: площадка ответила пустым телом — форма ответа изменилась")
    if full.get("draft"):
