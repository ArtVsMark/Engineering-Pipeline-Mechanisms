...
#:
#: НОМЕР БЕРЁТСЯ ПОСЛЕДНИЙ, А НЕ ВСЕ НАЙДЕННЫЕ, И ЭТО ПРЕДМЕТ (#582). Приписку
#: делает площадка, и делает её В КОНЕЦ темы — значит последний `(#N)` и есть
#: номер изменения, а всё, что стоит левее, написал автор.
#:
#: ЗАМЕР 21.09.2026, ИЗ-ЗА КОТОРОГО ПРАВКА И ПОЯВИЛАСЬ. Общая ветка покраснела
#: на слиянии #579: тема несла `(#551) (+2) (#579)` — номер задачи, написанный
#: автором, и номер изменения от площадки. Счёт по СУЩНОСТЯМ дал 14, счёт по
#: рёбрам графа — 13, и два независимых признака разошлись. До того из 520
#: уплотнений истории ДВА номера несло ровно одно — это.
...
def numbers_in(lines: list[str]) -> set[str]:
    ...
    ИЗ ТЕМЫ БЕРЁТСЯ ОДИН НОМЕР — ПОСЛЕДНИЙ. Тема описывает ОДНО принятое
    изменение, и второй номер в ней — это не второе изменение, а ссылка,
    написанная автором. Считать все найденные значило бы, что упоминание
    задачи в теме поднимает версию проекта
    ([005](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/005-hand-written-numbers-rot.md)).
    """
    found: set[str] = set()
    for line in lines:
        marks = PR_NUMBER_RE.findall(line)
        if marks:
            found.add(marks[-1])
...
def release_tag(root: Path | None = None) -> str | None:
    ...
    ближайший тег по образцу, и предрелизный вида `vX.Y.Z-rc1` под образец
    подходит, а под строгую форму — нет: ответом становилось «выпусков не видно
    вовсе», хотя рядом лежал настоящий выпуск. Один предрелизный тег обнулял бы
    версию проекта и значок. Нашёл внешний взгляд на #106.
