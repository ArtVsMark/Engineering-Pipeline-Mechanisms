...
#: Жирный заголовок пункта: `**5. Повторный взгляд.**`. Снимается перед
#: проверкой подлежащего — заголовок нумерует пункт внутри эпика и о задаче
#: ничего не говорит.
ITEM_HEADING_RE: Final = re.compile(r"^\s*\*\*[^*]+\*\*")

#: Пункт, чьё ПОДЛЕЖАЩЕЕ — задача: после заголовка и знаков препинания сразу
#: стоит ссылка. `#39 — внешний взгляд` и `**1. Решения** — #3` подходят;
#: `Последний открытый пункт #39: …` — нет.
SUBJECT_RE: Final = re.compile(r"^[\s—–\-:.,;]*#\d+\b")


def names_a_task(text: str) -> bool:
    """Пункт НАЗЫВАЕТ задачу собой, а не цитирует её по ходу прозы.

    Разница не стилистическая, и цена её измерена. Пункты 2, 5 и 6 эпика #196
    были отмечены сделанными 11.09.2026, не будучи сделанными: они называли
    `#3`, `#39` и `#15` — «пересмотр решения #3», «последний открытый пункт
    #39», «половина #15», — все три закрыты, и следование прочитало цитату как
    тождество. Эпик стал врать о собственном состоянии, а на нём висит выпуск
    единицы.

    Признак — ПОРЯДОК СЛОВ: подлежащее стоит первым. Пункт-ссылка открывается
    задачей, потому что задача и есть его предмет; пункт со своей работой
    открывается этой работой, а задача упоминается внутри. Это то же, что
    правило
    ...
    говорит про ссылки на правила: наличие ссылки — не основание, основание —
    её применимость.
    """
    return bool(SUBJECT_RE.match(ITEM_HEADING_RE.sub("", changerefs.outside_code(text))))


def linked_item(text: str) -> int | None:
    """Задача, которую пункт называет СОБОЙ, — если ровно одну; иначе ``None``.

    Два условия, и они закрывают разное. Ровно одна ссылка: «как в #52 и #3»
    говорит о своей работе, а задачи упомянуты. Ссылка подлежащим: одна
    ссылка внутри прозы — тоже упоминание, просто единственное. Нарушение
    любого значит «знать неоткуда», и механизм молчит (045: лучше не отметить,
    чем отметить неверно).
    """
    plain = changerefs.outside_code(text)
    numbers = {int(found["number"]) for found in TASK_REF_RE.finditer(plain)}
    if len(numbers) != 1 or not names_a_task(text):
        return None
