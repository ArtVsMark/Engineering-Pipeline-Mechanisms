...
#: Коды, за которыми стоит ВРЕМЕННЫЙ отказ площадки: запрос до неё дошёл и
#: сорвался по её вине. Обработан он при этом или нет — по коду не узнать: 502
#: и 504 отдаёт шлюз, не дождавшийся своего бэкенда, а тот мог успеть. Поэтому
#: список говорит лишь «повторять имеет смысл», а КОМУ повторять, решает
#: :func:`_survivable`. Список РАЗРЕШИТЕЛЬНЫЙ (068): что в нём не названо, тем
...
#: Метод, который СОЗДАЁТ сущность: комментарий, задачу, страницу выпуска.
#: Повтор его удваивает сделанное, и отличить «не обработано» от «обработано,
#: но ответ потерян» снаружи нечем.
CREATES: Final = "POST"
...
def _survivable(method: str, path: str, exc: Exception) -> bool:
    ...
    редакция повторяла любой 5xx, рассуждая так: «площадка сказала, что не
    обработала запрос». Для 502 и 504 это неверно по устройству: их отдаёт
    ШЛЮЗ, не получивший ответа от своего же бэкенда, — а бэкенд запрос мог
    выполнить. Повторённый `POST` тогда заводит второй комментарий или вторую
    задачу
    ([044](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/044-check-the-premise-before-fixing.md)).
    Нашёл внешний взгляд на #284 — тремя записями подряд.

    ПОЭТОМУ ГРАНИЦА ПРОХОДИТ ПО СОЗДАНИЮ, А НЕ ПО КОДУ. `PUT`, `PATCH` и
...
def request(
    ...
    if last is None:
        # Сюда попадают, только если попыток не делалось вовсе, — то есть при
        # `TRIES` меньше единицы. Прежняя редакция говорила здесь «ответа нет»,
        # и читатель понимал это как исход запроса, которого не бывает: запрос
        # либо вернул ответ, либо запомнил отказ. Причина названа настоящая, а
        # недостижимость держит прогон (замечание внешнего взгляда на #284).
        raise TransportError(f"{method} {path} → попыток не делалось: TRIES={TRIES}")
    try:
        raise last
