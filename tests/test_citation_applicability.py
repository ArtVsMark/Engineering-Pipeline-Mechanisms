...
#: НАЗВАННАЯ ГРАНИЦА: у ДВУХ ФОРМ ЦИТАТЫ РАЗНАЯ СТРОГОСТЬ, и это не упущение.
#: Ссылка с номером, которого в каталоге нет, краснеет у соседнего теста:
#: `rules/ru/999-` разрешиться не может ничем, кроме опечатки. Голое же число
#: отсеивается фильтром «это номер правила из нашего ответа» — и опечатка вроде
#: «(204)» уходит молча. Уравнять строгость нечем: в скобках ходят и коды
#: ответа площадки (401, 403, 429, 502, 503 нашлись в первом же прогоне), и
#: краснеть на них значило бы краснеть на законном
#: ([051](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/051-warn-on-likely-block-on-certain.md)).
#: Цена асимметрии названа, а не скрыта: опечатка в голом номере превращает
#: цитату в ничто, и поймать её может только чтение. Нашёл внешний взгляд на
#: #429.
