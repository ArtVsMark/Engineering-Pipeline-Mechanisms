...
`docs/agent/behaviour.md` опирался на 193 («приёмка починки строже чинимого дефекта»),
...
#:
#: ЗАПИСЬ НА НЕВЫПУЩЕННЫЙ ФРАГМЕНТ ЖУРНАЛА ЖИВЁТ ДО БЛИЖАЙШЕГО ВЫПУСКА, И ЭТО
#: НЕ ГИПОТЕЗА. Выпуск уносит `changelog.d/<имя>.md` в
#: `changelog.d/released/<номер>/`, то есть в `HISTORY`, которую гейт не
#: судит вовсе, — и разрешение остаётся без предмета. Замер 20.09.2026: выпуск
#: 1.2.0 покрасил общую ветку ровно одной такой записью из четырёх. Заводить
#: их не следует: голое упоминание правится В САМОМ фрагменте, пока он не
#: выпущен, а разрешение на временный файл бессмысленно по устройству.
