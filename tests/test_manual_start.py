...
#: Событие ВЫЗЫВАЕМОГО прогона. Такому кнопка не нужна не по исключению, а по
#: устройству: он не идёт сам никогда, его зовут по `uses:`, и рука у него
#: чужая — кнопка вызывающего. Своя кнопка не добавила бы способа добудиться,
#: зато сделала бы прогон идущим САМОСТОЯТЕЛЬНО, то есть завела бы вторую
#: проверку с тем же именем, которую пришлось бы отдельно объявлять в ответе.
#:
#: СПИСКОМ ИМЁН ЭТО НЕ ВЕДЁТСЯ. Перечислять вызываемые поимённо значит завести
#: второй список того же, и отстанет он на первом же вынесенном шаге
