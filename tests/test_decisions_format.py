...
#: Четыре части записи решения (042): контекст, само решение, рассмотренные
#: альтернативы и последствия. Заголовки канонические, а не «любые похожие»:
#: список синонимов превращает гейт формы в гейт фантазии автора, и первая же
#: запись с собственным именем раздела проходит его молча.
PARTS = {
    "Контекст": re.compile(r"^## Контекст\b", re.M),
    "Решение": re.compile(r"^## Решение\b", re.M),
    "Отвергнутые варианты": REJECTED_RE,
    "Последствия": re.compile(r"^## Последствия\b", re.M),
}


@pytest.mark.parametrize("path", records(), ids=lambda p: p.name)
def test_record_has_all_four_parts(path: Path) -> None:
    """Запись состоит из четырёх частей — иначе это объявление (042).

    Правило называет состав прямо: контекст, решение, **рассмотренные
    альтернативы**, последствия. Без контекста читатель не поймёт, на какой
    вопрос отвечали; без последствий — что запись с ним сделала. Альтернативы
    сверяет и соседний гейт по 161, и это не дубль: там предмет — отличить
    решение от хода работы, здесь — полнота состава записи.

    Замер 11.09.2026: из шести записей последствия назвали четыре. У 004 их не
    было вовсе, у 006 они стояли под собственным заголовком — то есть читатель,
    ищущий их на привычном месте, не находил ничего.
    """
    text = path.read_text(encoding="utf-8")
    missing = [name for name, mark in PARTS.items() if not mark.search(text)]
    assert not missing, f"{path.name}: записи не хватает частей: {', '.join(missing)}"
