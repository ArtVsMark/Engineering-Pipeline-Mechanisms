...
def test_a_standing_orphan_is_not_this_gates_subject(
    ...
    больше; нашёл их замер, а не этот гейт, и сняты они отдельными изменениями
    (#628 и следом). Краснеть на стоящем при каждом изменении значило бы учить
    себя обходить гейт; долг такого рода называется замером, а не красным на
    чужой работе (051).
    """
    monkeypatch.chdir(repo)
