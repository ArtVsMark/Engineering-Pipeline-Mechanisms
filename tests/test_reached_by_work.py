...
def test_a_standing_orphan_is_not_this_gates_subject(
    ...
    В дереве стояли две сироты, обе — от постройки соседа, делающего то же и
    больше; нашёл их замер, а не этот гейт, и сняты они порознь — #629 и #631,
    файлы не пересекались (133). Краснеть на стоящем при каждом изменении
    значило бы учить себя обходить гейт; долг такого рода называется замером, а
    не красным на чужой работе (051).
    """
    monkeypatch.chdir(repo)
