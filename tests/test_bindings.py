...
#: Оборот, которым причина называет ГРАНИЦУ — условие, после которого обобщение
#: заработает здесь. Словарь закрытый и объявлен списком: предикат по прозе иначе
#: не построить, и это названный предел, а не полнота
#: ([046](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/046-name-the-gaps-do-not-level-them.md)).
BORDER: Final = re.compile(
    r"вступит|станет ложн|перестанет быть|появ(ится|ятся)|как только|до тех пор|пока не",
    re.I,
)


def test_an_inapplicable_answer_names_its_border() -> None:
    """«Неприменимо» называет не только букву, но и ГРАНИЦУ своего отсутствия.

    Правило требует разложить запись НАДВОЕ: буква — предмет, названный дословно;
    обобщение — механизм поломки, который запись описывает. «Нет» законно, когда
    отсутствуют оба, — а значит ответ обязан сказать, при каком условии обобщение
    здесь заработает. Без этого «неприменимо» читается как «нас это не касается»
    и переживает то самое изменение проекта, которое его опровергает
    ([184](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/184-an-answer-of-not-applicable-is-split-in-two.md)).

    ЗАМЕР 18.09.2026: неприменимых тринадцать, границу называли двенадцать —
    требование исполнялось и не держалось ничем. Молчал о ней ровно один ответ, и
    правило, о котором он говорит, родилось у нас же
    ([002](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/002-rule-without-mechanism.md)).

    НОМЕР МОЛЧАВШЕГО ЗДЕСЬ НЕ НАЗВАН, И ЭТО НЕ СТЫДЛИВОСТЬ. Скобочный номер в
    прозе — форма ЦИТАТЫ правила, а цитата утверждает его применимость; правило,
    которое проект объявил неприменимым, процитировать нельзя, не противореча
    собственному ответу. Поймал это гейт цитат, а не вычитка
    ...
    ПРЕДЕЛ НАЗВАН: предикат ловит ОБОРОТ, а не мысль. Причина, назвавшая границу
    другими словами, будет отвергнута ложно — и лечится это дописыванием оборота,
    а не снятием проверки: словарь здесь закрытый и лежит рядом.
    """
    mute = [
        number
        for number, item in answers().items()
        if item["status"] == "not-applicable" and not BORDER.search(str(item.get("why") or ""))
    ]
    assert not mute, (
        "«неприменимо» без границы — не сказано, при каком условии обобщение заработает"
        f" здесь (184): {', '.join(mute)}\n  Оборот из объявленного словаря:"
        " «правило вступит…», «ответ станет ложным…», «пока не…»."
    )
...
#:
#: ГОЛАЯ ЦИФРА ПРЕДИКАТОМ НЕ СЧИТАЕТСЯ, И ЭТО ПОЧИНКА ПО НАХОДКЕ. Первая
#: редакция принимала любое `\d` — и ответ 066 прошёл гейт из-за числа «8» в
#: обороте «(шаг 8)», номера пункта дорожной карты. Замер по полосе: из
#: тринадцати «неприменимо» на голой цифре держался ровно ОДИН, и он же был
#: единственным без настоящего предиката. То есть послабление ничего не давало
#: и ровно одно пропускало (нашёл внешний взгляд на #455).
#:
#: ЧИСЛА ИЗ ПРИЗНАКА НЕ ИСЧЕЗЛИ: замер вроде «даёт ноль файлов» несёт их внутри
#: обратных кавычек либо рядом с адресом, и такой ответ проходит. Уходит ровно
#: цифра, стоящая в прозе сама по себе.
TREE_PREDICATE: Final = re.compile(
    r"[\w./-]+\.(?:py|md|json|ya?ml)|[\w.-]+/[\w./-]*|`[^`]+`|\bgit\s+\w+|\bgrep\b"
)
...
def test_an_event_alone_is_not_a_predicate() -> None:
    ...
    assert not TREE_PREDICATE.search("правило вступит вместе с очередью слияний (шаг 8)"), (
        "номер пункта дорожной карты принят за предикат — ровно находка #455"
    )
