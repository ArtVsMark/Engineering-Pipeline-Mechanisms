...
#: к тому же шагу (075).
#: ТРЕТЬЯ РЕДАКЦИЯ, И КАЖДАЯ ПРЕДЫДУЩАЯ ТЕРЯЛА СТРОКИ МОЛЧА. Первая требовала
#: `echo` в начале строки; вторая добавила ветвь `case` вида `3) echo …`;
#: третья добавила продолжение `|| echo …` и `&& echo …` — на нём жили ДВЕ
#: аннотации (`badges.yml`, `review.yml`), которых не судила ни одна проверка
#: этого файла. Нашёл их не человек: их назвал соседний тест полноты, заведённый
#: тем же заходом (075).
ANNOTATES: Final = re.compile(
    r'^\s*(?:(?:\|\||&&)\s*|[^\s()]+\)\s*)?echo\s+"::(?:error|warning)::(?P<said>.*)"\s*(?:;;)?\s*$'
)

#: ХВОСТ СОСЕДНЕЙ КОМАНДЫ, ПРИЛИПШИЙ К СООБЩЕНИЮ. Захват выше жадный, и на
#: строке вида `*) echo "::error::…"; exit "$code" ;;` он доводил сообщение до
#: ПОСЛЕДНЕЙ кавычки — то есть внутрь `exit "$code"`. Мера «сколько текста после
#: кода» считала при этом чужие знаки, и короткое сообщение могло пройти за
#: годное. Нашёл внешний взгляд на #397; замер по дереву: такая строка одна.
#:
#: РЕЖЕТСЯ ХВОСТ КОМАНДЫ, А НЕ ПО КАВЫЧКЕ. Сузить захват до `[^"]*` нельзя:
#: две живые аннотации несут кавычки ВНУТРИ подстановки
#: (`$(tr '\n' ' ' <"$said")`), и такой образец потерял бы их вовсе — гейт,
#: лишившийся предмета, хуже неточного (075).
COMMAND_TAIL: Final = re.compile(r'";\s*(?:exit|continue|break|return|shift)\b.*$')


def without_the_tail(said: str) -> str:
    """Сообщение аннотации без прилипшего хвоста соседней команды."""
    return COMMAND_TAIL.sub("", said)
...
def annotated() -> list[tuple[str, int, str]]:
    ...
    for path in sorted((ROOT / ".github" / "workflows").glob("*.yml")):
        for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            if said:
                found.append((path.name, number, without_the_tail(said["said"])))
    return found
...
def test_a_command_tail_is_not_part_of_the_message() -> None:
    """Хвост соседней команды в сообщение не входит.

    Захват аннотации жадный, и на строке `*) echo "::error::…"; exit "$code" ;;`
    он доводил сообщение до ПОСЛЕДНЕЙ кавычки — внутрь `exit`. Мера «сколько
    текста после кода» считала тогда чужие знаки, и короткое сообщение могло
    пройти за годное. Нашёл внешний взгляд на #397.
    """
    said = without_the_tail('остаток не прочитан (код $code): ${reason}"; exit "$code')
    assert said == "остаток не прочитан (код $code): ${reason}"


def test_quotes_inside_a_substitution_survive() -> None:
    """Кавычки ВНУТРИ подстановки сообщением остаются.

    Обратный конец: сузить захват до «до первой кавычки» значило бы потерять две
    живые аннотации, где причина берётся `$(tr '\\n' ' ' <"$said")`. Гейт,
    лишившийся предмета, хуже неточного (075).
    """
    whole = "Ответ не перенесён (код $rc): $(tr '\\n' ' ' <\"$said\")"
    assert without_the_tail(whole) == whole


def test_the_tail_cutter_found_its_subject() -> None:
    """Предмет есть: в дереве стоит строка, на которой хвост и прилипал."""
    tails = [
        f"{name}:{number}"
        for name, number, said in annotated()
        if '"; exit' in said or '"; continue' in said
    ]
    assert not tails, f"хвост команды всё ещё в сообщении: {tails}"


#: Аннотация, как её видно СЫРЫМ поиском: любая строка, где шаг зовёт `echo`
#: с меткой площадки. Грубее разбора выше и потому годится в мерило полноты.
RAW_ANNOTATION: Final = re.compile(r'echo\s+"::(?:error|warning)::')


def test_the_pattern_sees_every_annotation_in_the_tree() -> None:
    """Образец разбора видит ВСЕ аннотации дерева, а не часть.

    ОБРАЗЕЦ, ПОТЕРЯВШИЙ ПРЕДМЕТ, ЗЕЛЕНЕЕТ. Все проверки ниже идут по тому, что
    разобрал `ANNOTATES`: сузить его — и они начнут судить меньшее число строк,
    ничего не сказав. Проверено прямо: сужение захвата до «до первой кавычки»
    теряет две живые аннотации, где причина берётся `$(tr … <"$said")`, — и ни
    один тест на это не краснел
    ([075](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/075-a-guard-that-finds-nothing-must-fail.md)).

    МЕРИЛО ГРУБЕЕ РАЗБОРА НАМЕРЕННО: сырой поиск не умеет доставать сообщение,
    зато не умеет и промахнуться мимо строки. Два счёта об одной величине
    сверяются, и расхождение называется числом.
    """
    raw = [
        f"{path.name}:{number}"
        for path in sorted((ROOT / ".github" / "workflows").glob("*.yml"))
    ...
    assert raw, "аннотаций в дереве нет — предмета у проверки нет (075)"
    seen = {f"{name}:{number}" for name, number, _ in annotated()}
    lost = sorted(set(raw) - seen)
    assert not lost, (
        f"образец разбора не видит {len(lost)} аннотаций из {len(raw)}: {lost} — "
        "проверки ниже судят меньшее число строк и молчат об этом"
    )
