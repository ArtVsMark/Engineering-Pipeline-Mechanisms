...
def test_one_tool_is_bounded_the_same_way_everywhere() -> None:
    ...
    ПУСТОЙ ОТВЕТ ЗДЕСЬ НЕ ПРОВЕРЯЕТСЯ, И ЭТО НЕ ПРОБЕЛ. `needs()` либо отдаёт
    непустое отображение, либо бросает `NotRun` — «в прогонах нет ни одной
    строки установки» её же словами. Стоявший тут `assert found` был
    недостижим: он утверждал то, что вызов уже гарантировал, и выглядел
    проверкой, ничего не проверяя. Пометка `pragma: no cover` рядом была той же
    породы — покрытие в проекте не считается, и она не значила ничего. Обе нашёл
    внешний взгляд.
    """
    try:
        env.needs(ROOT)
    except env.NotRun as отказ:
