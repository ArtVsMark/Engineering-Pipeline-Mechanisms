...
def test_every_agent_call_names_its_model() -> None:
    ...
    ЗАМЕР 23.09.2026: все пять вызовов `claude-code-action` шли на умолчании
    действия, и на вопрос владельца «на какой модели ревью?» проект отвечал «не
    знаю». Ответил шаг `agent_run.py` первым же заходом: умолчание подписки —
    `claude-sonnet-5`, а не Opus. Умолчание меняется молча: сменилось бы оно —
    сменился бы ревьюер, и никто бы этого не увидел
    ...
    ЧТО ГЕЙТ НЕ ПРОВЕРЯЕТ — ЧТО МОДЕЛЬ ПРИМУТ. Первое объявление (#669)
    уронило взгляд: закреплённый CLI этого имени не знал. Принята ли модель,
    говорит аннотация шага `agent_run.py` на живом заходе — это держит
    `test_every_agent_call_names_its_model_and_its_failure` (#673).
    """
...
def test_all_agent_calls_name_one_model() -> None:
    ...
    держит этот гейт, а не ссылка на общую переменную: файлов прогонов три, и
    общей переменной между ними площадка не даёт
    ([022](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/blob/main/rules/ru/022-one-canonical-document.md)).

    ИСТОРИЯ ОДНОГО ОТКАТА (#673): первое объявление `claude-opus-5-5` уронило
    взгляд — действие v1.0.216 ставило CLI 2.1.261, в реестре моделей которого
    этого имени не было, и каждый заход кончался за 0–1 с. Действие поднято до
    v1.0.232 (CLI 2.1.280, имя в реестре есть), и модель объявлена заново.
    """
    said = declared_models()
    assert said, "в дереве нет ни одного вызова агента — проверять нечего (075)"
