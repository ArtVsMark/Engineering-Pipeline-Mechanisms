...
def test_all_agent_calls_name_one_model() -> None:
    ...
    «ОДНА» ВКЛЮЧАЕТ УМОЛЧАНИЕ ДЕЙСТВИЯ, и это отступление, а не норма. Гейт
    «каждый вызов объявляет модель» стоял рядом и снят вместе с объявлением:
    `--model claude-opus-5-5` действие v1.0.216 отвергло — каждый заход
    кончался за 0–1 с аннотацией «Claude result reported subtype success with
    is_error:true», и взгляд молчал на каждом изменении, а шаг с
    `continue-on-error` оставался зелёным. Проверить модель до слияния нечем:
    изменение, трогающее файл прогона, действие на той же голове не запускает.
    Какое имя модели действие примет, решает задача #673; до неё все вызовы
    идут на умолчании — но ВСЕ на одном.
    """
